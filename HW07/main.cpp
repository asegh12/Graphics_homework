...
void save_out_info(const std::string& filename)
{
  std::ofstream f;
  f.open(filename);
  ...
  for (int i = 0; i < models.size(); i++){
    glm::vec3 tmp = models[i].translate();
    f << model_names[i] << '\n' << models[i].scale()[0] << '\n';
    f << tmp[0] << ' ' << tmp[1] << ' ' << tmp[2] << '\n';
  }
...
void render_object()
{
  ...
  for (int i = 0; i < models.size(); ++i)
  {
    ...
    mat_normal = glm::mat3(glm::transpose(inverse(mat_model)));
    ...
    models[i].draw(loc_a_position, loc_a_normal, loc_u_diffuse_texture, loc_a_texcoord, loc_u_obj_ambient, loc_u_obj_diffuse, loc_u_obj_specular, loc_u_obj_shininess);
  }
